...
# Sidebar for About and Citation
st.sidebar.title("About")
st.sidebar.write(
    """
    This tool allows you to fetch molecule data, convert file formats, and visualize structures in 3D.

    Powered by:
    - Py3Dmol for Visualization
    - PubChem for fetching compounds
    - ASE for structure conversion
    """
)

st.sidebar.title("Citation")
st.sidebar.write("Sharma, M. & Mishra, D. (2019). J. Appl. Cryst. 52, 1449-1454.")
            ...
            with col3:
                try:
                    with open(sdf_files[i]) as f:
                        moldata = f.read()
                    viewer = py3Dmol.view(width=1600, height=400)
                    viewer.addModel(moldata, 'sdf')
                    viewer.setStyle({'stick': {}})
                    viewer.zoomTo()
                    viewer_html = viewer._make_html()
                    st.components.v1.html(viewer_html,width=1600, height=800)
                except Exception as e:
